...
[English](README.en.md) · [文档中心](docs/README.md) · [当前设计](docs/current-design.md) · [快速入门](docs/getting-started.zh-CN.md) · [架构概览](docs/architecture.zh-CN.md) · [K12 Expert](docs/experts/k12.zh-CN.md) · [Query Runtime API](docs/api/query-runtime.zh-CN.md)
...
> **Public Launch Candidate / 公开发布候选准备中**
>
> KnowMesh 仍是 `0.1.0-alpha`，但仓库正在按公开发布候选标准收口：README、公开样例、Issue/PR 模板、Security、OpenAPI、release gate、CI、CodeQL、Scorecard 和 artifact checksum 都必须能被外部贡献者复核。分享预览图位于 `assets/social/knowmesh-social-preview.png`。
>
> Public Beta 之前还会固定首次使用路径、浏览器样例流、Integration Examples 合约、Expert / Provider lifecycle 和 release evidence。扩展阶段按 `official` -> `certified` -> `community` -> `experimental` 标注，避免贡献者误把实验入口当成稳定 API。
...
想快速理解 KnowMesh 的实际边界，可以先看 [examples/public-samples](examples/public-samples/README.md)：

- `general-docs`：虚构运维手册，用来验证 Query Runtime、引用、拒答、反馈、包预览和版本 manifest。
- `operations-handbook`：非 K12 Expert SDK 样例，用制度、流程、复核节奏、回滚规则和证据要求验证扩展合同。
- `k12-synthetic`：合成的小学五年级数学样例，用来验证 K12 Expert 结构和范围，不包含真实教材内容。

这些样例无密钥、不上传、不调用 OCR、不调用 embedding、不写向量索引、不创建云资源。
...
- Expert 注册表、K12 schema、对象关系契约和 95% TOC / 100% 越界拒答门槛。
- 文档维护、反馈 review、版本记录、诊断导出。
- release smoke、artifact install smoke、package boundary gate。
- `0.5.0 Provider Adapters` release evidence：provider manifests、parser/OCR boundary、embedding/vector boundary、provider diagnostics browser proof、no-cloud public path 和 package asset review。
- `0.6.0 Integration SDK` release evidence：endpointManifestReadiness、sdkClientProof、examplesDriftProof、integrationSafetyProof、providerAwareNoCloudProof 和 integrationPackageAssetReview。
- `0.7.0 Consumer Integration Proof` release evidence：installedSdkConsumerProof、livePublicSampleSdkProof、integrationRecipeProof、privacyBoundaryAuditProof、providerAwareNoCloudConsumerProof 和 consumerPackageAssetReview。
- `0.8.0 Operator Workflow Proof` release evidence：sourceIntakeProof、executionRecoveryProof、maintenanceTargetedRerunProof、versionRollbackProof、operatorBrowserWorkflow、operatorPrivacyAuditProof 和 operatorPackageAssetReview。
- `0.9.0 First-Run Usability Proof` release evidence：firstRunLaunchProof、guidedSetupProof、buildRecoveryProof、firstQuestionProof、maintenanceNextActionProof、firstRunBrowserWorkflow 和 firstRunPackageAssetReview。
- `1.0.0 Usable Product Proof` release evidence：usableLaunchReliabilityProof、usableDocumentIntakeProof、usableWebConsoleWorkflowProof、usableDurableDataPackageProof、usableBrowserWorkflow、usablePrivacyProof 和 usableProductPackageAssetReview。
...
- [K12 Expert](docs/experts/k12.zh-CN.md)
- [Expert Authoring Kit](docs/experts/authoring.zh-CN.md)
- [Query Runtime API](docs/api/query-runtime.zh-CN.md)
- [Operator Workflow Proof](docs/operator-workflow.zh-CN.md)
- [First-Run Usability Proof](docs/first-run-usability.zh-CN.md)
- [Usable Product Proof](docs/usable-product.zh-CN.md)
- [应用集成指南](docs/integrations.zh-CN.md)
- [Integration Examples](examples/integrations/README.md)
- [Provider 适配器](docs/providers.zh-CN.md)
- [Community Backlog](docs/community-backlog.zh-CN.md)
- [Release Operations](docs/release-operations.zh-CN.md)
...
- [Release Candidate Evidence](docs/release-candidate.zh-CN.md)
...
建议仓库 Topics：`knowledge-base`, `rag`, `retrieval-augmented-generation`, `local-first`, `document-ai`, `document-intelligence`, `sqlite`, `sqlite-first`, `vector-search`, `citations`, `auditable-ai`, `knowledge-graph`, `education`, `k12`, `textbook`, `aliyun`, `dashscope`, `nodejs`, `web-console`, `open-source`。
